...
impl ChipsHost {
  pub fn new(config_path: &Path) -> anyhow::Result<ChipsHost> {
    let config = serde_json::from_reader(File::open(config_path)?)?;

    let mut swap_secret = [0; 32];
    OsRng.fill_bytes(&mut swap_secret);
    Ok(ChipsHost {
      engine: ChipsEngine::new(),
      rpc: ChipsRpc::new(&config)?,
      #[cfg(test)]
      refund_pubkey: None,
      refund_pubkey_script: ChipsEngine::decode_address(&config.refund)?,
      address: None,
  ...
  async fn prepare_refund_and_spend(&mut self, lock_id: Txid, lock_value: u64) -> anyhow::Result<(u64, Vec<u8>)> {
    ...
    let spend = ChipsEngine::prepare_spend(
      refund.txid(),
...
#[async_trait]
impl ScriptedHost for ChipsHost {
  ...
  fn generate_deposit_address(&mut self) -> String {
    let address = ChipsEngine::generate_deposit_address();
  ...
  fn override_refund_with_random_address(&mut self) {
    ...
    self.refund_pubkey_script = ChipsEngine::decode_address(
      &Address::p2pkh(
        &pubkey,
        NETWORK
      ).to_string()
