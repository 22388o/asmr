...
use crate::{
  ...
  coins::{
    ScriptedVerifier,
    chips::{engine::*, rpc::ChipsRpc}
  }
};

pub struct ChipsVerifier {
  engine: ChipsEngine,
  rpc: ChipsRpc,
  destination: String,
...
impl ChipsVerifier {
  pub fn new(config_path: &Path) -> anyhow::Result<ChipsVerifier> {
    ...
    Ok(ChipsVerifier {
      ...
      destination_script: ChipsEngine::decode_address(&config.destination)?,
...
#[async_trait]
impl ScriptedVerifier for ChipsVerifier {
  ...
  async fn verify_and_wait_for_lock(
    ...
    ChipsEngine::verify_meta_and_inputs(lock.clone())?;
